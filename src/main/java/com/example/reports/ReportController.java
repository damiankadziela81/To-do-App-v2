...
class ReportController {
    ...
    ResponseEntity<TaskDoneBeforeDeadline> readTasksWithDeadline (@PathVariable int id) {
        return taskRepository.findById(id)
    ...
    public static class TaskDoneBeforeDeadline {
        public String description;
        public boolean done;
        ...
        TaskDoneBeforeDeadline(final Task task, final List<PersistedTaskEvent> events) {
            ...
            lastOccurrence = events.stream().map(event -> event.occurrence).max(LocalDateTime::compareTo).orElse(null);
            if(task.getDeadline()==null || !task.isDone()) doneBeforeDeadline = false;
            else doneBeforeDeadline = lastOccurrence.isBefore(task.getDeadline());
        }
    }
}
